...
- a new battle's chart starts with `#OFFSET:0` and `#BPMS:0=120` as a stand-in. while its timing is still that (untouched since the editor opened, with no stops, scroll speed changes, notes or dialogue lines on beats), the editor's "tempo change here" sets the song's tempo from the play position, the way "set bpm" and "first beat here" would. kept in front of the song's tempo, the 120 bpm would be the chart's top tempo, and speed mod would scroll the song's start faster than the rest. undo, then the same change again, keeps the 120.
