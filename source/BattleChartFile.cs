...
internal static class BattleChartFile
{
    ...
    internal static string? Check(string text, int lanes, int[] noteCounts, List<string> notices)
    {
        ...
        try
        {
            // The chart the arcade plays: notes before the song starts (0:00 in the audio) are left
            // out of it. Beat 0 before the song (a positive #OFFSET) is fine by itself.
            var baked = ChartOffset.Bake(parsed.PlayableSong(slots));
            if (baked.Dropped > 0) notices.Add(ChartOffset.DroppedText(baked.Dropped));
        }
