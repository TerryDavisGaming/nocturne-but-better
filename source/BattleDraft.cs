...
internal sealed class BattleDraft
{
    ...
    /// <summary>The lines during the song, in the order written, for the chart editor (items that aren't lines are left out).</summary>
    internal List<DialogueCue> DuringCues() =>
        (Section(DialogueSection.During) ?? new JsonArray()).OfType<JsonObject>().Select(CueOf).ToList();
