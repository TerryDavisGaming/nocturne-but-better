...
internal static partial class BattleCreator
{
    ...
    // How high the box sits in a battle: the Narrator's is raised, and a character's line during
    // the song is raised as the battle raises it, away from the receptors (except in 2D upscroll,
    // where they're at the top). The Narrator's box stays at its own height then too.
    private const float NarratorLift = 110, LiveLift = BattleDialogue.LiveBubbleHeight;
    ...
    private static Shot PreviewShot()
    {
        ...
        shot.Lift = live && !shot.Narration && SettingsState.Mode != ScrollMode.Upscroll2D ? LiveLift : lift;
