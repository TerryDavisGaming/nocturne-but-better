...
internal static partial class ChartEditor
{
    ...
    private static string ScrollsTag() =>
        string.Join(",", scrolls.Select(s => $"{s.Beat.ToString("0.######", CultureInfo.InvariantCulture)}={s.Ratio.ToString("0.###", CultureInfo.InvariantCulture)}"));
