...
internal static class CustomCharts
{
    ...
    internal static SongData? FindSong(string name)
    {
        ...
        // Rebuilt on a miss, since songs load and unload as the game goes. Custom battles' runtime
        ...
        foreach (var song in Resources.FindObjectsOfTypeAll<SongData>())
            if (song && !CustomBattles.IsRuntimeName(song.name) && !songs.ContainsKey(song.name)) songs[song.name] = song;
    ...
    /// Custom battles' runtime SongData count as shared too: they take no custom charts, which also
    /// keeps them out of the chart editor's song list.
    /// </summary>
    internal static bool SharesScore(SongData song)
    {
        if (CustomBattles.IsRuntimeName(song.name)) return true;
        ...
        foreach (var other in Resources.FindObjectsOfTypeAll<SongData>())
            if (other && other.name != song.name && !CustomBattles.IsRuntimeName(other.name) && other.HighScoreKey == key) return true;
        return false;
    }
    ...
    internal static int WriteGameCharts()
    {
        ...
        foreach (var song in Resources.FindObjectsOfTypeAll<SongData>())
        {
            if (!song || CustomBattles.IsRuntimeName(song.name) || !done.Add(song.name)) continue;
            var maps = song.beatmaps;
